...
//
// *** Object Locator Tracker decoder for JavaScript ***
// support parser type: Object Locator Tracker
//
// Author: Brian.Jiang
// Version v1.0.1
//
...
function decodepayload(input) {
    ...
    if(input.fport==136){ // Object Locator Tracker Sensor
      var a = input.payload;
      ...
      var button_st = indust_binary.substring(7, 8); // button trigger 
      var moving_st = indust_binary.substring(6, 7); // moving mode
      var gnssfix_st = indust_binary.substring(4, 5); // no GNSS fix
      var gnsserr_st = indust_binary.substring(3, 4); // GNSS error
      ...
      console.log('Sensor Type: Object Locator Tracker');
      console.log('DeviceEUI: '+input.deveui);
      console.log('button trigger event: '+button_st); //1 - button trigger event, 0 - no button trigger event
      console.log('moving mode: '+moving_st); //1 - moving mode, 0 - stationary mode
      console.log('no GNSS fix: '+gnssfix_st); //1 - no GNSS fix, 0 - GNSS fixed
      console.log('GNSS error: '+gnsserr_st); //1 - GNSS error, 0 - GNSS OK
